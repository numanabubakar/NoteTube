        ...
        if (videoData) {
          ...
          if (sessionData) {
            ...
            const { data: existingStats } = await supabase
              .from('learning_stats')
              .select('id, session_count, videos_watched')
              .eq('user_id', user.id)
              .eq('stats_date', today)
              .single();
    ...
    try {
      ...
      if (user) {
        ...
        if (saveError) {
          ...
          if (existingStats) {
            await supabase.from('learning_stats').update({
              notes_created: (existingStats.notes_created || 0) + 1,
              updated_at: new Date().toISOString()
            }).eq('id', existingStats.id);
          } else {
            await supabase.from('learning_stats').insert({
    ...
    try {
      ...
      if (user) {
        ...
        if (saveError) {
          ...
          // Record quiz completion in session (using a placeholder score for now or actual score later)
          // For now we just mark that a quiz was generated by hitting the stats endpoint or direct update
          const today = new Date().toISOString().split('T')[0];
          const { data: existingStats } = await supabase
            .from('learning_stats')
          ...
          if (existingStats) {
            await supabase.from('learning_stats').update({
              quizzes_taken: (existingStats.quizzes_taken || 0) + 1,
              updated_at: new Date().toISOString()
            }).eq('id', existingStats.id);
          } else {
            await supabase.from('learning_stats').insert({
